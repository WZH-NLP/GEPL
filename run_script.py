...
def validation(args, span_model, type_model, tokenizer, id_to_label_span, pad_token_label_id, best_dev, test, best_dev_bio, test_bio,\
    ...
    best_dev, best_dev_bio, _ = evaluate(devs, args, span_model, type_model, tokenizer, \
    ...
    test, test_bio, is_updated_test = evaluate(tests, args, span_model, type_model, tokenizer, \
    ...
    if args.local_rank in [-1, 0] and is_updated_test:
        path = os.path.join(args.output_dir, "checkpoint-best-span-test")
        ...
        path = os.path.join(args.output_dir, "checkpoint-best-type-test")
    ...
    return best_dev, test, best_dev_bio, test_bio, is_updated_test
...
def train(args, train_dataset_src, train_dataset, id_to_label_span, id_to_label_type_src, id_to_label_type_tgt, tokenizer, pad_token_label_id):
    ...
    # [span_model, type_model], [optimizer_span, optimizer_type] = amp.initialize([span_model, type_model], [optimizer_span, optimizer_type], opt_level="O1")  # 这里是“欧一”，不是“零一”
    # type_model, optimizer_type = amp.initialize(type_model, optimizer_type, opt_level="O1")  # 这里是“欧一”，不是“零一”
    ...
    for epoch in train_iterator:
        ...
        best_dev, test, best_dev_bio, test_bio, _ = validation(args, span_model, type_model, tokenizer, \
                                                               id_to_label_span, pad_token_label_id, best_dev, test,
                                                               best_dev_bio, test_bio, \
                                                               global_step, t_total, epoch, devs, tests)
        ...
        for step, batch_src in enumerate(epoch_iterator):
            ...
            # print("----",len(batch_src), "-----")
            # print(batch_src)
            inputs = {"input_ids": batch_src[0], "attention_mask": batch_src[1], "ori_mask": batch_src[6], "labels_bio": batch_src[2],"mlm_labels": batch_src[8],"graph_prompt_labels":batch_src[9], "prompt_label_idx":batch_src[7], "graph_prompt": True,"tgt": False, "reduction": "none"}#------
            outputs_span_src = span_model(**inputs)
            inputs = {"input_ids": batch_src[0], "attention_mask": batch_src[1], "ori_mask": batch_src[6], "labels_type": batch_src[3], "logits_bio": outputs_span_src[2], "mlm_labels": batch_src[8],"graph_prompt_labels":batch_src[9], "prompt_label_idx":batch_src[7], "graph_prompt": True, "tgt": False, "reduction": "none"}
            outputs_type_src = type_model(**inputs)
            # Entity Span Detection Sub-task Source Domain + dual loss re-weighting
            loss1 = span_model.loss(outputs_span_src[0], outputs_type_src[1], tau=args.tau_span, eps=args.eps_span)
            # Entity Type Classification Sub-task Source Doamin + dual loss re-weighting
            loss2 = type_model.loss(outputs_type_src[0], outputs_span_src[1], tau=args.tau_type, eps=args.eps_type)
            # Linguistic Consistency
            loss6 = share_loss(outputs_span_src[-2], outputs_type_src[-2], loss_funct, args.L)#------

            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "ori_mask": batch[6], "labels_bio": batch[2], "mlm_labels": batch[8],"graph_prompt_labels":batch[9], "prompt_label_idx":batch[7], "graph_prompt": True, "tgt": True, "reduction": "none"}#------
            outputs_span = span_model(**inputs)
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "ori_mask": batch[6], "labels_type": batch[3], "logits_bio": outputs_span[2], "mlm_labels": batch[8],"graph_prompt_labels":batch[9], "prompt_label_idx":batch[7], "graph_prompt": True, "tgt": True, "reduction": "none"}
            outputs_type = type_model(**inputs)
            # Entity Span Detection Sub-task Target Domain + dual loss re-weighting
            ...
            inputs = {"inputs_embeds": permute_embed, "attention_mask": batch[1], "ori_mask": batch[6],"labels_bio": batch[2], "tgt": True, "reduction": "mean"}#------
            outputs_span_ = span_model(**inputs)
            loss31 = outputs_span_[0]
            # Entity Type Classification Sub-task Source Doamin
            ...
            inputs = {"inputs_embeds": permute_embed, "attention_mask": batch[1], "ori_mask": batch[6],"labels_type": batch[3],"graph_prompt": False, "permute": True, "tgt": True, "reduction": "mean"}
            outputs_type_ = type_model(**inputs)
            loss41 = outputs_type_[0]
            # Linguistic Consistency
            loss7 = share_loss(outputs_span[-2], outputs_type[-2], loss_funct, args.L)#-------

            # print(outputs_type_src[3])
            # Intermediate augmented domain
            loss5 = type_model.mix_up(outputs_type_src[3], outputs_type[3], batch_src[3], batch[3], args.alpha, args.beta)
            loss_mlm = outputs_type_src[-1] + 3 * outputs_type[-1] + outputs_span_src[-1] + 3 * outputs_span[-1]
            loss = loss1+loss2+loss3+loss4+0.1*loss5+0.05*(loss6+loss7)+0.1*(loss31+loss41)+0.5*loss_mlm # ALL
            ...
            # #混合精度训练
            # with amp.scale_loss(loss, [optimizer_span,optimizer_type]) as scaled_loss:
            #    scaled_loss.backward()

            # with amp.scale_loss(loss2, optimizer_type) as scaled_loss:
            #    scaled_loss.backward()


            tr_loss += loss.item()

            # _, _, _, _, _ = validation(args, span_model, type_model, tokenizer,
            #                                                        id_to_label_span, pad_token_label_id, best_dev, test,
            #                                                        best_dev_bio, test_bio,
            #                                                        global_step, t_total, epoch, devs, tests)
...
def main():
    ...
    # _, eval_dataset = load_and_cache_examples(args, tokenizer, pad_token_label_id, mode="dev")
    # _, eval_dataset = load_and_cache_examples(args, tokenizer, pad_token_label_id, mode="test")
    # Training

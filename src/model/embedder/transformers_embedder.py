...
class TransformersEmbedder(nn.Module):
    ...
    def __init__(self, transformer_model_name: str,
        ...
        # transformer_model_name='./pretrained/bert-base-cased-literature'
        self.model = context_models[model_name]["model"].from_pretrained(transformer_model_name,
                                                                                   output_hidden_states= output_hidden_states,
                                                                                   return_dict=False)
        # self.tokenizer = context_models[transformer_model_name]["tokenizer"].from_pretrained(transformer_model_name)
        # self.model.resize_token_embeddings(len(self.tokenizer)+2)
    ...
    def forward(self, word_seq_tensor: torch.Tensor,
                       orig_to_token_index: torch.LongTensor, ## batch_size * max_seq_len
        ...
        _, max_sent_len = orig_to_token_index.size()# max_sent_len表示字的长度
        return torch.gather(word_rep[:, 1:, :], 1, orig_to_token_index.unsqueeze(-1).expand(batch_size, max_sent_len, rep_size))#去除了cls token，并且把原输入对应字的embedding取出来

...
def parse_arguments(parser):
    ...
    parser.add_argument('--prompt', type=str, default=None, choices=["max", "random", "sbert", "bertscore"], help="prompt mode")
    parser.add_argument('--template', type=str, default=None, choices=["no_context", "context", "context_all", "structure", "structure_all",'lexical','lexical_all'], help="template mode")
    parser.add_argument('--graph_prompt', type=int, choices=[1,0], help="graph prompt mode")
...
def train_model(config: Config, epoch: int, train_loader: DataLoader, dev_loader: DataLoader, test_loader: DataLoader, result_out_file_path: str):
    ...
    for i in tqdm(range(1, epoch + 1), desc="Epoch"):
        ...
        for iter, batch in tqdm(enumerate(train_loader, 1), desc="--training batch", total=len(train_loader)):
            optimizer.zero_grad()
            # print(type(batch.prompt_label_idx, batch.mlm_labels, ))
            # print(batch.graph_prompt_labels,batch.prompt_label_idx)
            loss = model(words = batch.input_ids.to(config.device), word_seq_lens = batch.word_seq_len.to(config.device),
                    ...
                    mlm_labels = batch.mlm_labels.to(config.device), graph_labels = batch.graph_labels.to(config.device),
                    graph_prompt_labels=batch.graph_prompt_labels.to(config.device),prompt_label_idx=batch.prompt_label_idx.to(config.device))#
...
def main():
    ...
    if opt.mode == "train":
        ...
        bert_base_cased_path = './pretrained/bert-base-cased'
        tokenizer = context_models[conf.embedder_type]["tokenizer"].from_pretrained(bert_base_cased_path)
        print(colored(f"[Data Info] Reading dataset from: \n{conf.train_file}\n{conf.dev_file}\n{conf.test_file}", "blue"))
        conf.train_file = conf.train_all_file
        if conf.prompt is None:
            ...
            if conf.search_pool == "source":
                source_data = TransformersNERDataset('dataset/conll/train.txt', tokenizer, number=conf.train_num,
        ...
        tokenizer = context_models[saved_config.embedder_type]["tokenizer"].from_pretrained(bert_base_cased_path)
